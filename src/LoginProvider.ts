...
export class LoginProvider {
    ...
    static sendEmail(type: string, userid: string, to: string, subject: string, text: string, span: Span): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            ...
            text = text.split("\n").join("<br/>\n");
            let html = text + `\n\n<img src="${imgurl}" alt="isread" border="0" width="1" height="1">`
    ...
    static async config(): Promise<any> {
        ...
        const res2 = {
            ...
            web_auto_login: Config.web_auto_login,
    ...
    static async post_AddTokenRequest(req: any, res: any, next: any): Promise<void> {
        ...
        try {
            ...
            if (Util.IsNullEmpty(name == "") && Util.IsNullEmpty(app == "")) {
                name = "autogenerated for openrpa"
                app = "openrpa";
            }
            await Logger.DBHelper.AddRequestTokenID(key, {name, app}, span);
            ...
            if (!Util.IsNullEmpty(key)) {
                if (req.user) {
                    ...
                    if (!Util.IsNullUndefinded(exists)) {
                        ...
                        exists.jwt = await Auth.User2Token(user, Config.longtoken_expires_in, span);
                        await Logger.DBHelper.AddRequestTokenID(key, exists, span);
                        try {
