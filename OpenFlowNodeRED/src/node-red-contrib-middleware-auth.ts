...
export class noderedcontribmiddlewareauth {
    ...
    public static async process(socket:WebSocketClient, req:express.Request,res:express.Response,next:express.NextFunction):Promise<void> {
        ...
        if (login && password) {
            try {
                ...
                if(result.user!=null) {
                    ...
                    if(allowed.length === 1) {
                        ...
                        console.log("noderedcontribmiddlewareauth: Authorized " + user.username + " for " + req.url);
                        req.user = cacheduser.user;
                        req.user.jwt = cacheduser.jwt;
                        return next();
                    } else {
            ...
            console.log("noderedcontribmiddlewareauth: Unauthorized, no username/password for " + req.url);
        }
        res.statusCode = 401;
